...
describe("POST IMAGE 👉 GET POST SORT BY DISLIKES", () => {
    ...
    test("It should display an error if the token is not valid.", async () => {
        ...
        const { body } = await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: fake_token }).send(__mocks__1.QUERY_GET_BY_DESC);
    ...
    test("It should display an error if the token is not sent.", async () => {
        const { body } = await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").send(__mocks__1.QUERY_GET_BY_DESC);
        expect(body?.errors[0].message).toBe(`Autenticación no valida, vuelva a iniciar sesión`);
        expect(body?.data).toBeNull();
    });
    test("Deberia retornar lista de post", async () => {
        ...
        __mocks__1.QUERY_CREATE_POST.variables.variables.description = 'black blue post';
        await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: token }).send(__mocks__1.QUERY_CREATE_POST);
        __mocks__1.QUERY_CREATE_POST.variables.variables.description = 'blue post';
        await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: token }).send(__mocks__1.QUERY_CREATE_POST);
        __mocks__1.QUERY_CREATE_POST.variables.variables.description = 'orange post';
        await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: token }).send(__mocks__1.QUERY_CREATE_POST);
        __mocks__1.QUERY_GET_BY_DESC.variables.description = 'blue';
        const { body } = await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: token }).send(__mocks__1.QUERY_GET_BY_DESC);
        console.log(body);
    ...
    test("Deberia retornar lista 0 post", async () => {
        const res = await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").send(__mocks__1.QUERY_CREATE_USER);
        let token = res.body?.data.createUser.token;
        __mocks__1.QUERY_GET_BY_DESC.variables.description = 'other';
        const { body } = await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: token }).send(__mocks__1.QUERY_GET_BY_DESC);
        expect(body?.data.getPostByDesc).toHaveLength(0);
        await (0, supertest_1.default)(__mocks__1.URL_SERVER).post("/").set({ Authorization: token }).send(__mocks__1.QUERY_DELETE_USER);
    });
});
//# sourceMappingURL=getPostByDesc.test.js.map